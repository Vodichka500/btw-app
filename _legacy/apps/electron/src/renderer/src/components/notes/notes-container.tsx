...
export function NotesContainer({ isVisible, isFullScreen, onToggleMode, onClose }: NotesContainerProps): ReactNode {
  // Блокировка скролла
  useEffect(() => {
    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          ...
          <div
            className={cn(
              'flex items-center justify-between px-4 h-12 shrink-0 border-b bg-muted/30 select-none'
            )}
          >
            <div className="flex items-center gap-2 text-sm font-medium text-foreground/80">
              {isFullScreen && (
                <Button variant="ghost" size="icon" className="h-8 w-8 mr-1" onClick={onToggleMode}>
