...
import { SortableSnippetCard } from '@/components/features/snippet/sortable-snippet-card'
...
export function SnippetGrid({ onCreateSnippet, onEditSnippet }: SnippetGridProps) {
  const [isReordering, setIsReordering] = useState(false)
  const [searchQuery, setSearchQuery] = useState('') // 🔥 Теперь локальное состояние

  const { viewMode, selectedCategoryId, setViewMode } = useUIStore()
        ...
        <div>
          ...
          <p className="text-sm font-medium text-muted-foreground mt-1">
            {snipsQuery.isLoading
              ? 'Ładowanie...'
              : `${filteredSubCategories.length > 0 ? filteredSubCategories.length + ' foldery, ' : ''}${filteredSnippets.length} snippety`}
          </p>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                  {filteredSubCategories.map((cat) => (
                    <div
                      key={cat.id}
                      onClick={() => setViewMode('category', cat.id)} // 🔥 Используем глобальный сеттер
